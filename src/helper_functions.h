...
/*
 * Struct representing nearest neighbor assignment 
 * between predicted observation measurement and the map landmark
 */
...
inline LandmarkObs particle_transform_lidar_to_map(double px,double py, double ptheta, double lidar_x, double lidar_y){
	LandmarkObs tObs;
	tObs.x = lidar_x*cos(ptheta)-lidar_y*sin(ptheta)+px;
	tObs.y = lidar_x*sin(ptheta)+lidar_y*cos(ptheta)+py;

	return tObs;
}

/*
	* Computes the gaussian probability between a predicted position and a map landmark
	* @param x			x coordinate of map landmark
	* @param y			y coordinate of map landmark
	* @param mu_x		x coordinate of predicted landmark observation
	* @param mu_y		x coordinate of predicted landmark observation
	* @param std_x	standard deviation of lidar measurement in x direction
	* @param std_y	standard deviation of lidar measurement in y direction
	*
	* returns  (double)gaussian probability
	* Added by Kofi Baafi
*/
inline double gaussian_probability(double x, double y, double mu_x, double mu_y,
														double std_x,double std_y){
	double exp_x = pow((x-mu_x),2)/(2*pow(std_x,2));
  double exp_y = pow((y-mu_y),2)/(2*pow(std_y,2));

  double prob = ((1/(2*(M_PI*std_x*std_y)))*exp(-(exp_x+exp_y)));
  return prob;
}

/*
	* Computes the multivariate probability of a set of map landmarks and a particle's predicted position
	* @param
	* @param
	* Added by Kofi Baafi
*/
inline double particle_multivariate_location_probability(std::vector<LandmarkAssociation> assocs, double std_meas[]){
	double mv_prob = 1.0;
	double gauss_prob;

	for (auto& i:assocs){
		//std::cout<<"x:"<<i.predicted_x<<" "<<i.predicted_y<<" "<<i.map_x<<" "<<i.map_y<<std::endl;
		gauss_prob = gaussian_probability(i.predicted_x,i.predicted_y,i.map_x,i.map_y,std_meas[0],std_meas[1]);
		//std::cout<<"gprob:"<<gauss_prob<<std::endl<<std::endl;
		mv_prob*=gauss_prob;
		
	}

	//std::cout<<"mv_prob: "<<mv_prob<<std::endl<<std::endl;
